...
void gradHist(
    float* M, float* O, float* H, int h, int w, int bin, int nOrients, int softBin, bool full)
{
    ...
    for (x = 0; x < w0; x++) {
        ...
        gradQuantize(
            O + x * h, M + x * h, O0, O1, M0, M1, nb, h0, sInv2, nOrients, full, softBin >= 0);
...
void hogChannels(
    float* H, const float* R, const float* N, int hb, int wb, int nOrients, float clip, int type)
{
#define GETT(blk)                                                                                  \
    t = R1[y] * N1[y - (blk)];                                                                     \
    if (t > clip)                                                                                  \
...
void hog(float* M, float* O, float* H, int h, int w, int binSize, int nOrients, int softBin,
    bool full, float clip)
{
    float *N, *R;
...
void fhog(
    float* M, float* O, float* H, int h, int w, int binSize, int nOrients, int softBin, float clip)
{
...
{
    const size_t dims[3] = { (size_t)h, (size_t)w, (size_t)d }, n = h * w * d;
    int b;
    mxArray* M;
    if (id == mxINT32_CLASS)
        b = sizeof(int);
...
void checkArgs(int nl, mxArray* pl[], int nr, const mxArray* pr[], int nl0, int nl1, int nr0,
    int nr1, int* h, int* w, int* d, mxClassID id, void** I)
{
    int nDims;
    if (nl < nl0 || nl > nl1)
    ...
    auto dims = mxGetDimensions(pr[0]);
    *h = dims[0];
    *w = dims[1];
    *d = (nDims == 2) ? 1 : dims[2];
    *I = mxGetPr(pr[0]);
